...
def getText(url):
    for list_num in range(1, getPageNum(url)):  # 从第一页到第getPageNum(url)页
        ...
        em_list = page.find_all('em')  # 匹配em内容 球号
        div_list = page.find_all('td', {'align': 'center'})  # 匹配 <td align=center>这样的内容 日期

        # 初始化n
        n = 0
        # 将双色球数字信息写入num.txt文件
        fp = open(file_path.format("num.txt"), "w")
        for div in em_list:   #循环获得丢好
            #emnum1 = div.get_text()
            #print (emnum1)
            text = div.get_text()
        ...
        # 注意事项：数据替换 去除‘ and ()" 符号  在记事本中替换掉！
        fp = open(file_path.format("hun.txt"), "a")  #追加模式，若打开的是已有文件则直接对已有文件操作，若打开文件不存在则创建新文件，只能执行写（追加在后面），不能读。即追加写。
